...
class Configuration(private val mContext: Context, private val provider: String) {
    ...
    private fun readConfiguration() {
        val file = when (provider) {
            ...
            "google" -> R.raw.google
            "safefleetdev" -> R.raw.safefleetdev
            else -> R.raw.safefleettest
        }
    ...
    internal fun getRequiredConfigWebUri(propName: String): Uri {
        ...
        if (TextUtils.isEmpty(scheme) || !("http" == scheme || "https" == scheme || "navigator" == scheme )) {
            throw InvalidConfigurationException(
                "$propName must have navigator for scheme"
            )
        }
